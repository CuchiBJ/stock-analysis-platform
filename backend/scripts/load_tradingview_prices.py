#!/usr/bin/env python3
"""Load prices for TradingView stocks using Yahoo Finance"""
import sys
...
async def main():
    ...
    async with async_session() as db:
        # Get TradingView stocks (those with market_cap from TradingView but no prices)
        ...
        logger.info(f"Found {len(symbols)} TradingView stocks without prices")
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                for date, row in hist.iterrows():
                    volume = int(row['Volume']) if row['Volume'] < 2147483647 else 2147483647  # Cap at int32 max
                    await db.execute(
                        text("""
                ...
                await db.commit()
                logger.info(f"[{i+1}/{len(symbols)}] Loaded prices for {symbol} ({len(hist)} days)")
